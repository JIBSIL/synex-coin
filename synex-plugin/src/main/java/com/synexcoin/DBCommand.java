...
public class DBCommand implements CommandExecutor {
    ...
    public DBCommand(SynexCoinPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (
                plugin.hasPermission(sender, "synexcoin.db")
        ) {
            if (args.length == 1) {
                ...
                sender.sendMessage(plugin.chatPrefix+"/synex:db [connect|disconnect|status]");
            }
        } else {
