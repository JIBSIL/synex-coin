...
public class RetryExport implements Runnable {
    ...
    public RetryExport(SynexCoinPlugin plugin, Player sender, double amount, Transaction tx, String signature) {
        this(plugin, sender, amount, tx, signature, 1, 60, 2);
    }

    public RetryExport(SynexCoinPlugin plugin, Player sender, double amount, Transaction tx, String signature, int maxRetry, int retryTimeoutInMinutes, int confirmTimeoutInMinutes) {
        this.plugin = plugin;
        this.tx = tx;
        this.sender = sender;
        this.amount = amount;
            ...
            if (retryAttempt > this.maxRetry) {
                if (confirmCheckCount > this.confirmTimeoutInMinutes) {
                    ...
                    try {
                        ...
                        if(!this.plugin.rpcClient.getApi().isBlockhashValid(this.tx.getBlockhash())) {
                            ConfirmedTransaction confirmedTX = this.plugin.rpcClient.getApi().getConfirmedTransaction(this.signature);
                            if (confirmedTX == null) {
                        ...
                        if (confirmedTX.getMeta().getErr() != null) {
                            try {
                                ...
                                if (serverBalance.subtract(new BigDecimal(amount)).doubleValue() < 0) {
                                    ...
                                    this.plugin.sendURLToPlayer(this.sender, "Check the Transaction Status", "https://solscan.io/tx/"+signature, SynexCoinPlugin.TELLRAWCOLOR.yellow);
                                    retryAttempt++;
                                }
                            } catch (Exception e) {
                                errorCount++;
