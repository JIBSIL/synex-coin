...
public class SendCommand implements CommandExecutor {
    ...
    public SendCommand(SynexCoinPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (
                plugin.hasPermission(sender, "synexcoin.send")
        ) {
            if (args.length >= 2) {
                    ...
                    if (amount > 0) {
                        if (senderBalance >= amount) {
                            if (args.length == 2) {
                                ...
                                sender.sendMessage(plugin.chatPrefix+"/synex:send <player> <amount> confirm");
                                return true;
                            }
